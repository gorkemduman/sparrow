defmodule Sparrow.HNS.V1.Pool.Supervisor do
  @moduledoc """
  Supervises a single HNS workers pool.
  """
  use Supervisor
  ...
  def init(raw_config) do
    pool_configs =
      Enum.map(raw_config, fn single_config ->
        {single_config[:path_to_json], get_hns_pool_config(single_config)}
      end)

    for {path_to_json, {pool_config, _pool_tags}} <- pool_configs do
      Sparrow.HNS.V1.ProjectIdBearer.add_project_id(
        path_to_json,
    ...
    children =
      for {{_json, {pool_config, pool_tags}}, index} <-
        ...
        id = String.to_atom("Sparrow.Hns.Pool.ID." <> Integer.to_string(index))

        IO.inspect(pool_config)
        %{
          id: id,
          start:
            {Sparrow.H2Worker.Pool, :start_link, [pool_config, :hns, pool_tags]}
        }
      end
  ...
  @spec get_hns_pool_config(Keyword.t()) ::
          {Sparrow.H2Worker.Pool.Config.t(), [atom]}
  defp get_hns_pool_config(raw_pool_config) do
    uri = Keyword.get(raw_pool_config, :endpoint, @hns_default_endpoint)
